...
def plotline1():
    ...
    for move in line1:
        if move[0] == "R":
            move = move.replace("R", "")
            x = 0
            currentpos = allline1coords[-1]
            currentxpos = str(currentpos).split(":")[0]
            currentypos = str(currentpos).split(":")[1]
            for x in range(0, int(move)):
                currentxpos = int(currentxpos) + 1
                stepcount1 = stepcount1 + 1
                allline1coords.append(str(currentxpos) +":"+ str(currentypos)+ ":" + str(stepcount1))
            ...
            for x in range(0, int(move)):
                ...
                stepcount1 = stepcount1 + 1
                allline1coords.append(str(currentxpos) +":"+ str(currentypos)+ ":" + str(stepcount1))
    print("Finished Plotting Line 1, Plotted "+str(stepcount1)+" steps")
...
def plotline2():
    ...
    for move in line2:
        if move[0] == "R":
            move = move.replace("R", "")
            x = 0
            currentpos = allline2coords[-1]
            currentxpos = str(currentpos).split(":")[0]
            currentypos = str(currentpos).split(":")[1]
            for x in range(0, int(move)):
                currentxpos = int(currentxpos) + 1
                stepcount2 = stepcount2 + 1
                allline2coords.append(str(currentxpos) +":"+ str(currentypos)+ ":" + str(stepcount2))
            ...
            for x in range(0, int(move)):
                ...
                stepcount2 = stepcount2 + 1
                allline2coords.append(str(currentxpos) +":"+ str(currentypos)+ ":" + str(stepcount2))
    print("Finished Plotting Line 2, Plotted "+str(stepcount2)+" steps in total")

def checkforintercepts():
    global smallestdistance
    global leaststeps
    for coord in allline1coords:
        coordwithoutstep = ((str(coord).split(":")[0]) +":"+ (str(coord).split(":")[1]))
        line1steps = str(coord).split(":")[2]
        if coordwithoutstep in allline2coordswithoutsteps:
            print("INTERCEPT FOUND - "+coord)
            for val in allline2coords:
                if val.find(coordwithoutstep) != -1:
                    line2steps = str(val).split(":")[2]
            xpos = str(coord).split(":")[0]
            ypos = str(coord).split(":")[1]
            stepsneeded = int(line1steps) + int(line2steps)
            dist = abs(int(xpos)) + abs(int(ypos))
            print("Distance = "+str(dist))
            print("Steps to this point = "+str(stepsneeded))
            if dist != 0:
                if int(dist) < int(smallestdistance):
                    smallestdistance = int(dist)
            if int(stepsneeded) != 0:
                if int(stepsneeded) < int(leaststeps):
                    leaststeps = stepsneeded

    print("Comparison Complete - Smallest distance found was: "+ str(smallestdistance))

def plot2withoutsteps():
    for coord in allline2coords:
        xpos = str(coord).split(":")[0]
        ypos = str(coord).split(":")[1]
